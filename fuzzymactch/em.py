...
df = pd.read_csv("./tmp_ciq_data.csv")
df.head()
    ...
    for i in range(gp_start, gp_end+1):
        ...
        for j in range(i+1, gp_end+1):
            if pd.isna(all_main_name['alias'].iloc[j]):
                fuzz_socre = fuzz.token_sort_ratio(
                    all_main_name['names'].iloc[i], all_main_name['names'].iloc[j])
        ...
        if i % (len(all_names)//1000) == 0:
            ...
            if process > 0:
                ...
                str = "progress: %.2f" % process + "% " + 'Current cost %s' % cost + \
                    " mins " + 'Remain cost %s' % remain + " mins "
                print(str)
                # print('Remain cost',)
